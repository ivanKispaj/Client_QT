...
uint EncodePassword::cycle_shift_left(uint val, int bit_count)
{
    return (val << bit_count | val >> (32 - bit_count));
}

uint EncodePassword::bring_to_human_view(uint val)
{
    return ((val & 0x000000FF) << 24) | ((val & 0x0000FF00) << 8) | ((val & 0x00FF0000) >> 8)
           | ((val & 0xFF000000) >> 24);
}

void EncodePassword::sha1(std::string &value)
{
    const uint H[5] = {0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0};
    ...
    uint needAdditionalBytes = one_block_size_bytes
    ...
    memcpy(newMessage, &value[0], msize_bytes);
    ...
    *ptr_to_size = EncodePassword::bring_to_human_view(msize_bytes * 8);
    ...
    for (int i = 0; i < totalBlockCount; i++)
    {
        ...
        {
            exp_block[j] = EncodePassword::bring_to_human_view(block[j]);
        }
        // следующие 64...
        for (int j = one_block_size_uints; j < block_expend_size_uints; j++)
        {
            exp_block[j] = exp_block[j - 3] ^ exp_block[j - 8] ^ exp_block[j - 14]
                           ^ exp_block[j - 16];
    ...
    // A,B,C,D,E являются выходными 32б составляющими посчитанного хэш
    std::ostringstream ostr;
    ostr << std::hex << A << std::hex << B << std::hex << C << std::hex << D << std::hex << E;
    value = ostr.str();
}
